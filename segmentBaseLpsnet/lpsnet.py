...
class LPSNet2Path(BaseSegNet):
    ...
    def forward(self, x):
        ...
        xl = torch.cat([xh, upsample(xl, (int(xh.shape[-2]), int(xh.shape[-1])))], dim=-3)
        xl = self.head(xl)
        return xl  # 调整大小
...
def get_lspnet_s(deploy=False, num_classes=19):
    depth = (1, 3, 3, 10, 10)
    width = (8, 24, 48, 96, 96)
    resolution = (3 / 4, 1 / 4, 0)
    net = get_lpsnet(depth, width, resolution, deploy=deploy, num_classes=num_classes)
    return net


def get_lspnet_m(deploy=False, num_classes=19):
    depth = (1, 3, 3, 10, 10)
    width = (8, 24, 48, 96, 96)
    resolution = (1, 1 / 4, 0)
    net = get_lpsnet(depth, width, resolution, deploy=deploy, num_classes=num_classes)
...
def get_lspnet_l(deploy=False, num_classes=19):
    depth = (1, 3, 3, 10, 10)
...
if __name__ == "__main__":
    ...
    model = get_lspnet_l(num_classes=21)
